...
* `btn-outline` — прозрачная контурная кнопка со светлой рамкой;
...
### Растяжка-фичер в посте

Оранжевый фон под растяжкой. Внутри можно использовать любые `html`-теги.

```html
<div class="ficher">Разметка и контент внутри фичера</div>
```
